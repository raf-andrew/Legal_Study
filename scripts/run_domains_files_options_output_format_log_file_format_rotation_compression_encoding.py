...
logger = logging.getLogger("run_domains_files_options_output_format_log_file_format_rotation_compression_encoding")
...
class DomainFileOptionOutputFormatLogFileFormatRotationCompressionEncodingRunner:
    ...
    async def run(
        ...
        """Run in specific mode with specific configuration and specific domains and files and specific options and specific output and specific format and specific log level and specific log file and specific log format and specific log rotation and specific log compression and specific log encoding.
    ...
    async def _run_sniffing(
        ...
        try:
            ...
            if self.output_format == "json":
                with open(self.output_path, "w", encoding=self.log_encoding) as f:
                    json.dump(result, f, indent=2)
            elif self.output_format == "yaml":
                with open(self.output_path, "w", encoding=self.log_encoding) as f:
                    yaml.dump(result, f, indent=2)
            elif self.output_format == "csv":
                with open(self.output_path, "w", newline="", encoding=self.log_encoding) as f:
                    writer = csv.writer(f)
...
def main() -> None:
    ...
    try:
        ...
        # Run in specific mode with specific configuration and specific domains and files and specific options and specific output and specific format and specific log level and specific log file and specific log format and specific log rotation and specific log compression and specific log encoding
        global runner
        runner = DomainFileOptionOutputFormatLogFileFormatRotationCompressionEncodingRunner(args.mode, args.config, domains, files, options, args.output, args.format, args.log_level, args.log_file, args.log_format, args.log_max_bytes, args.log_backup_count, args.log_compress, args.log_encoding)
