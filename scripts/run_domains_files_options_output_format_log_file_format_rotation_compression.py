...
logger = logging.getLogger("run_domains_files_options_output_format_log_file_format_rotation_compression")
...
class DomainFileOptionOutputFormatLogFileFormatRotationCompressionRunner:
    ...
    async def run(
        ...
        """Run in specific mode with specific configuration and specific domains and files and specific options and specific output and specific format and specific log level and specific log file and specific log format and specific log rotation and specific log compression.
    ...
    async def _run_sniffing(
        ...
        try:
            ...
            logger.info(f"Running sniffing in {self.mode} mode with config {self.config_path}, domains {self.domains}, files {self.files}, options {self.options}, output {self.output_path}, format {self.output_format}, log level {self.log_level}, log file {self.log_file}, log format {self.log_format}, log max bytes {self.log_max_bytes}, log backup count {self.log_backup_count}, and log compress {self.log_compress}...")
...
def main() -> None:
    ...
    try:
        ...
        # Run in specific mode with specific configuration and specific domains and files and specific options and specific output and specific format and specific log level and specific log file and specific log format and specific log rotation and specific log compression
        global runner
        runner = DomainFileOptionOutputFormatLogFileFormatRotationCompressionRunner(args.mode, args.config, domains, files, options, args.output, args.format, args.log_level, args.log_file, args.log_format, args.log_max_bytes, args.log_backup_count, args.log_compress)
