"""
Script to run sniffing infrastructure in debug mode.
"""
import argparse
import asyncio
import logging
...
class DebugRunner:
    """Class for running sniffing infrastructure in debug mode."""
    ...
    async def run(
        ...
        try:
            ...
            logger.error(f"Error running debug mode: {e}")
            raise
...
def handle_signal(signum, frame):
    ...
    """
    logger.info("Stopping debug mode...")
    if runner:
        runner.stop()
...
def main() -> None:
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Run sniffing infrastructure in debug mode"
        )
        parser.add_argument(
            "--domains",
            type=str,
