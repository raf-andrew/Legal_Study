...
class ConfigRunner:
    """Class for running sniffing infrastructure in a specific mode with a specific configuration."""
    ...
    async def run(
        ...
        """Run in specific mode with specific configuration.
        ...
        try:
            ...
            logger.error(f"Error running in {self.mode} mode with config {self.config_path}: {e}")
            raise
    ...
    async def _run_sniffing(
        ...
        try:
            ...
            logger.info(f"Running sniffing in {self.mode} mode with config {self.config_path}...")
...
def main() -> None:
    ...
    try:
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler("run_config.log"),
        ...
        # Run in specific mode with specific configuration
        global runner
        runner = ConfigRunner(args.mode, args.config)
