...
logger = logging.getLogger("run_domains_files_options_output_format_log_file_format_rotation_compression_encoding_mode_delay_buffering")

class CompressedRotatingFileHandler(RotatingFileHandler):
    """Class for handling compressed rotating log files."""

    def __init__(self, filename: str, mode: str = "a", maxBytes: int = 0, backupCount: int = 0, encoding: Optional[str] = None, delay: bool = False, compress: bool = False, buffering: int = -1):
        """Initialize handler.
...
class DomainFileOptionOutputFormatLogFileFormatRotationCompressionEncodingModeDelayBufferingRunner:
    ...
    async def _run_sniffing(
        ...
        try:
            ...
            logger.info(f"Running sniffing in {self.mode} mode with config {self.config_path}, domains {self.domains}, files {self.files}, options {self.options}, output {self.output_path}, format {self.output_format}, log level {self.log_level}, log file {self.log_file}, log format {self.log_format}, log max bytes {self.log_max_bytes}, log backup count {self.log_backup_count}, log compress {self.log_compress}, log encoding {self.log_encoding}, log mode {self.log_mode}, log delay {self.log_delay}, and log buffering {self.log_buffering}...")
