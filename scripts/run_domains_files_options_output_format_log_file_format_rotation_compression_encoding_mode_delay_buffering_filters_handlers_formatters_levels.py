...
logger = logging.getLogger("run_domains_files_options_output_format_log_file_format_rotation_compression_encoding_mode_delay_buffering_filters_handlers_formatters_levels")
...
def get_log_handlers(
    ...
    Args:
        ...
        log_levels: Dictionary of log levels
    ...
    if args.log_file:
        ...
        handler.setLevel(log_levels.get("file", logging.INFO))
    ...
    if args.syslog_host:
        ...
        handler.setFormatter(log_formatter)
        handler.setLevel(log_levels.get("syslog", logging.INFO))
        handlers.append(handler)
    ...
    if args.smtp_host:
        ...
        handler.setFormatter(log_formatter)
        handler.setLevel(log_levels.get("smtp", logging.ERROR))
        handlers.append(handler)
    ...
    if args.http_host:
        ...
        handler.setFormatter(log_formatter)
        handler.setLevel(log_levels.get("http", logging.ERROR))
        handlers.append(handler)
    ...
    if args.stdout:
        ...
        handler.setFormatter(log_formatter)
        handler.setLevel(log_levels.get("stdout", logging.INFO))
        handlers.append(handler)

    return handlers

class DomainFileOptionOutputFormatLogFileFormatRotationCompressionEncodingModeDelayBufferingFiltersHandlersFormattersLevelsRunner:
...
def main() -> None:
    ...
    try:
        ...
        parser.add_argument(
            "--log-levels",
            type=str,
            help="Comma-separated list of log levels to use in handler=level format"
        )
        parser.add_argument(
            "--syslog-host",
            type=str,
        ...
        log_levels = {}
        if args.log_levels:
            for level in args.log_levels.split(","):
                handler, level = level.split("=")
                log_levels[handler] = getattr(logging, level.upper())

        # Get log handlers
        log_handlers = get_log_handlers(args, log_filters, log_formatter, log_levels)
        ...
        # Run in specific mode with specific configuration and specific domains and files and specific options and specific output and specific format and specific log level and specific log file and specific log format and specific log rotation and specific log compression and specific log encoding and specific log mode and specific log delay and specific log buffering and specific log filters and specific log handlers and specific log formatters and specific log levels
        global runner
        runner = DomainFileOptionOutputFormatLogFileFormatRotationCompressionEncodingModeDelayBufferingFiltersHandlersFormattersLevelsRunner(args.mode, args.config, domains, files, options, args.output, args.format, args.log_level, args.log_file, args.log_format, args.log_max_bytes, args.log_backup_count, args.log_compress, args.log_encoding, args.log_mode, args.log_delay, args.log_buffering, log_filters, log_handlers, log_formatter, log_levels)
